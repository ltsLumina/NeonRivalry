Shader "TransitionsPlus/DoubleSlide"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float side = pow(_T / (abs(0.5 - uv.x) + 0.0001), 4 * _Contrast);
                side *= 1.0 + ComputeNoise(i.noiseUV) * 10.0;

                float2 direction = float2(1.0, 0);
